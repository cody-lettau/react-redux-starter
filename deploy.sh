...
# 2. Install NPM packages necessary for building
...
eval $NPM_CMD install --no-progress --production
...
# set NODE_ENV=dev
eval $NPM_CMD run build-client-debug-windows

# set NODE_ENV=production
eval $NPM_CMD run build-server-windows
eval $NPM_CMD run build-client-windows

exitWithMessageOnError "npm run build failed"
echo "Finished building project!"

# 4. Copy build folder to deployment_target...
...
#5. Update based on built files?
echo "Adjusting node version and configs"
selectNodeVersion
